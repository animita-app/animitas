...
create policy "Users can create revisions for sites they can edit"
  on heritage_site_revisions for insert
  with check (
    exists (
      ...
      and (
        auth.uid() = creator_id
        or auth.uid() in (select id from profiles where role in ('editor', 'superadmin'))
      )
    )
  );
